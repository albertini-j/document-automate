...
- Document-to-file association uses case-insensitive matching on the pattern DOCUMENT NUMBER 1 + "_" + VERSION within filenames (excluding the transmittal log itself).
...
- DATE stored in document_list.xlsx reflects the analysis/approval timestamp (current runtime), overriding the DATE provided in the transmittal log after validation.
- transmittal_database.xlsx stores both the DATE from the transmittal log (LOG DATE) and the processing DATE (approval timestamp).
...
LOG_DATE_HEADER = "LOG DATE"
DATABASE_HEADERS = EXPECTED_HEADERS + [LOG_DATE_HEADER, "FILENAMES"]
REQUIRED_FIELDS = {
    "TRANSMITTAL NUMBER",
    "DATE",
    "ITEM",
...
def find_matching_files(transmittal_dir: Path, doc_number: str, version: str, log_file: Path) -> List[str]:
    matches: List[str] = []
    target = f"{doc_number}{VERSION_SEPARATOR}{version.lower()}"
    for entry in transmittal_dir.iterdir():
        if entry == log_file or entry.is_dir():
            continue
        if target in entry.name.lower():
...
def append_to_database(db_path: Path, rows: List[TransmittalRow]) -> None:
    ...
    for row in rows:
        ...
        values.append(row.raw.get(LOG_DATE_HEADER))
    ...
    try:
        ...
        for idx, raw in enumerate(raw_rows, start=2):
            try:
                ...
                log_date = coerce_date(raw["DATE"])
                raw[LOG_DATE_HEADER] = log_date
                raw["DATE"] = log_date
                filenames = find_matching_files(
                    transmittal_dir, normalized_doc, normalized_version, log_file
                )
